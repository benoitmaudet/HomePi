...
/**
 * <[>WiringPi raspberrypi.GPIO Control</[>
 * 
 * <p>
...
public class Gpio {
    ...
    /**
     * The total number of raspberrypi.GPIO pins available in the WiringPi library.
     * <i>(Note this is not the maximum pin count on the Pi raspberrypi.GPIO header.)</i>
     */
    public static final int NUM_PINS = 20;

    /**
     * raspberrypi.GPIO pin constant for INPUT direction for reading pin states
     * 
     * @see #pinMode(int,int)
     */
    ...
    /**
     * raspberrypi.GPIO pin constant for OUTPUT direction for writing digital pin states (0/1)
     * 
     * @see #pinMode(int,int)
     */
    ...
    /**
     * raspberrypi.GPIO pin constant for PWM_OUTPUT direction for writing analog pin states
     * 
     * @see #pinMode(int,int)
     */
    ...
    /**
     * raspberrypi.GPIO pin state constant for LOW/OFF/0VDC
     * 
    ...
    /**
     * raspberrypi.GPIO pin state constant for HIGH/ON/+3.3VDC
     * 
     * @see #digitalWrite(int,int)
     */
    ...
    /**
     * raspberrypi.GPIO constant to disable the pull-up or pull-down resistor mode on a raspberrypi.GPIO pin.
     * 
    ...
    /**
     * raspberrypi.GPIO constant to enable the pull-down resistor mode on a raspberrypi.GPIO pin.
     * 
     * @see #waitForInterrupt(int,int)
     */
    ...
    /**
     * raspberrypi.GPIO constant to enable the pull-up resistor mode on a raspberrypi.GPIO pin.
     * 
     * @see #waitForInterrupt(int,int)
     */
     ...
     * a mapping from virtual pin numbers 0 through 16 to the real underlying Broadcom raspberrypi.GPIO pin
     * numbers. See the pins page for a table which maps the wiringPi pin number to the Broadcom
     * raspberrypi.GPIO pin number to the physical location on the edge connector.
     * </p>
     * 
     ...
     *         initialization of the raspberrypi.GPIO has failed. A return value of '0' indicates a successful
     ...
     * accessing the hardware directly. This can be called as a non-root user provided the raspberrypi.GPIO pins
     * have been exported before-hand using the gpio program. Pin number in this mode is the native
     * Broadcom raspberrypi.GPIO numbers.
     * </p>
     * 
     * <p>
     * <ul>
     ...
     * to use the Broadcom raspberrypi.GPIO pin numbers directly with no re-mapping.
     * </p>
     * 
     * <p> <b><i>This function needs to be called with root privileges.</i></b></p>
     * 
     ...
     * pin 1 (raspberrypi.GPIO-18) supports PWM output. The pin number is the number obtained from the pins
     * table.
     * </p>
     * 
     ...
     * @see <a
     *      href="https://projects.drogon.net/raspberry-pi/wiringpi/functions/">https://projects.drogon.net/raspberry-pi/wiringpi/functions/</a>
     * @param pin The raspberrypi.GPIO pin number. </br><i>(Depending on how wiringPi was initialized, this may
     *            be the wiringPi pin number or the Broadcom raspberrypi.GPIO pin number.)</i>
     ...
     * @param pin The raspberrypi.GPIO pin number. </br><i>(Depending on how wiringPi was initialized, this may
     *            be the wiringPi pin number or the Broadcom raspberrypi.GPIO pin number.)</i>
     * @return If the selected raspberrypi.GPIO pin is HIGH, then a value of '1' is returned; else of the pin is
     ...
     * 
     * <p>
     * With a newer kernel patched with the raspberrypi.GPIO interrupt handling code, you can now wait for an
     * interrupt in your program. This frees up the processor to do other tasks while you're waiting
     * for that interrupt. The raspberrypi.GPIO can be set to interrupt on a rising, falling or both edges of
     * the incoming signal.
     * </p>
     ...
     * 
     * <p>
     * Before you call waitForInterrupt, you must first initialize the raspberrypi.GPIO pin and at present the
     * only way to do this is to use the gpio program, either in a script, or using the system()
     * call from inside your program.
     * </p>
     * 
     * <p>
     * e.g. We want to wait for a falling-edge interrupt on raspberrypi.GPIO pin 0, so to setup the hardware, we
     * need to run:
     * 
     * <pre>
     * gpio edge 0 falling
     ...
     * <p>[Hardware]</p>
     * 
     * <p> This method provides the edge raspberrypi.GPIO pin number for the requested wiringPi pin number.  </p>
     * 
     * @see <a
     *      href="https://projects.drogon.net/raspberry-pi/wiringpi/functions/">https://projects.drogon.net/raspberry-pi/wiringpi/functions/</a>
     * @return The return value represents the raspberrypi.RaspberryPi raspberrypi.GPIO (edge) pin number.
    ...
    // homepi.System.out.println( "Running Clean Up..." );
    // }
    // }
}
